...
def test_find_restoraint(client):
    ...
    # выцепляем меню со списком элементов и ищем еду
    ...
    # элемент не сразу подгружается, нужно подождать. после чего, ищем Рукколу
    ...
    # проверяем элементы Рукколы на наличие
    ...
    # разворачиваем превью
    preview = client.driver.find_element_by_id(MAPS_ME.pp__preview)
    wait_element(
        client, MAPS_ME.pp__preview, active_element=preview, condition=lambda: preview.location['y'] != 960
                 )
    client.driver.swipe(500, 1200, 500, 0)
    client.driver.swipe(500, 1200, 500, 0)

    # проверяем элементы превью
    check_simple_sequence(client, [
        MAPS_ME.today_opening_hours, MAPS_ME.tv__place_website, MAPS_ME.tv__place_cuisine
    ])

    # проверяем телефон и координаты на соответствие
    ...
    # свапаем координаты на другой вид и проверяем на соответствие
